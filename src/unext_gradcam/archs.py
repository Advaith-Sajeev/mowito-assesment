...
try:
    ...
    class ConvModule(nn.Module):
        """Minimal drop-in for mmcv.cnn.ConvModule: conv -> norm -> act."""
        def __init__(self, in_channels, out_channels, kernel_size,
            ...
            self.norm = None
            if use_norm:
                ntype = (norm_cfg or {}).get('type', 'BN')
                if ntype in ('BN','BatchNorm','BN2d','BatchNorm2d'):
                    self.norm = nn.BatchNorm2d(out_channels)
                elif ntype in ('SyncBN','SyncBatchNorm'):
                    self.norm = nn.SyncBatchNorm(out_channels)
                else:
                    raise ValueError(f'Unsupported norm_cfg: {norm_cfg}')
            atype = (act_cfg or {'type':'ReLU'}).get('type','ReLU')
            Act = _ACT.get(atype, nn.ReLU)
            try:
                self.act = Act(inplace=inplace)
            except TypeError:
                self.act = Act()
...
class UNext(nn.Module):
    ...
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=224, patch_size=16, in_chans=3,  embed_dims=[ 128, 160, 256],
        ...
        # classification head: pool bottleneck features and output 1 logit (binary)
        self.cls_in_channels = embed_dims[2]
        self.cls_head = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(self.cls_in_channels, 1)
        )
    ...
    def forward(self, x):
        ...
        # capture bottleneck features for classification head
        bottleneck_feat = out
        ...
        seg_logits = self.final(out)
        # classification logit (raw)
        cls_logit = self.cls_head(bottleneck_feat).squeeze(1)
